...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/12 21:57:48 by iidkhebb          #+#    #+#             */
/*   Updated: 2021/12/12 22:25:24 by iidkhebb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_parser(t_maps *map)
{
	...
	while (map->map_load[++y])
	{
		...
		{
			put_img(map->map_load[y][x], x * 50, y * 50, map);
			if (map->map_load[y][x] == 'P')
			{
				map->playerx = x;
				map->playery = y;
			}
			if (map->map_load[y][x] == 'M')
			{
				map->enemy_x = x;
				map->enemy_y = y;
			}
...
void	ft_render(t_maps *map)
{
	...
	else
	{
		...
		ft_parser(map);
		ft_render_initial_text(map);
		mlx_loop_hook(map->mlx, mov_enemy, map);
		mlx_loop(map->mlx);
	}
}
